...
update bnpid_old.CandidateInformationShared set classgroup = 'Senior Women', classtype = 'Panawagan' where classtype like '%SR WOMEN%';
update bnpid_old.CandidateInformationShared set classnumber = 176, classtype = 'Panawagan' where classtype = 'PANAWAGAN176';

update bnpid_old.CandidateInformationShared
set classtype = 'Panawagan'
where classtype is null
   or classtype like '%PANAWAGAN%'
   or classtype in
      ('PANAWAGAN', 'PANAWGAN', 'SPECIAL CLASS', 'YM PANAWAGAN', 'YM CLASS', 'YOUTH MINISTRY', 'YOUNT WOMEN', 'SR MEN', 'SR WOMENS PANAWAGAN', 'YM', 'PANWAGAN',
       'JR GIRLS', 'SENIOR MEN', 'PAANWAGAN', 'YOUNG MEN', 'Retreat Class', 'PANAWGAGAN', 'Panawagqn', 'JR PANAWAGAN', 'Retreat', '14', 'YOUNG MEN PANAWAGAN',
       'PAPAWAGAN', 'SENIOR WOMEN', 'YOUTH', 'OUTH MINISTRY', 'YOUTH GIRLS', ' SENIOR WOMEN', 'SENIOR', 'YM SPECIAL CLASS', 'YOUNG WOMEN', 'test');

update bnpid_old.CandidateInformationShared
set classtype = 'Apostolic'
where classtype in ('AMC', 'APOTOSLIC CLASS', 'APOTOLIC MISSION CLASS', 'APOLSTOLIC MISSION', 'APOSTOLIC MISSION');

update bnpid_old.CandidateInformationShared
set classtype = 'Mission'
where classtype like '%mission%'
   or classtype in ('MISSIO');

update bnpid_old.CandidateInformationShared
set classtype = 'Regular'
where classtype like '%REGULAR%'
   or classtype in ('REGULAR RETREAT');

update bnpid_old.CandidateInformationShared
set classtype = 'Little Lambs'
where classtype like '%LITTLE LAMBS%';

update bnpid_old.CandidateInformationShared
set classtype = 'Spiritual Workshop'
where classtype like '%SPIRITUAL%'
   or classtype in ('SPRITUAL WORKSHOP', 'WORKSHOP');

update bnpid_old.CandidateInformationShared
set classtype = 'Marian'
where classtype like '%MARIAN%';

update bnpid_old.CandidateInformationShared
set classtype = 'Munting Pagsasanay'
where classtype like '%MUNTING PAGSASANAY%';

-- (2) Fix classgroup mapping
update bnpid_old.CandidateInformationShared
set classgroup = 'Young Men'
where classgroup in ('YOUNG MEN', 'YOUTH MEN');

update bnpid_old.CandidateInformationShared
set classgroup = 'Young Women'
where classgroup in ('YOUNG WOMEN', 'YM GIRLS');

update bnpid_old.CandidateInformationShared
set classgroup = 'Youth (Mixed)'
where classgroup in ('YOUTH MINISTRY', 'YOUTH MIXED', 'YM', 'LITTLE LAMBS', 'YOUTH');

update bnpid_old.CandidateInformationShared
set classgroup = 'Junior Boys'
where classgroup in ('JUNIOR BOYS', 'JR BOYS', 'JR. BOYS');

update bnpid_old.CandidateInformationShared
set classgroup = 'Junior Girls'
where classgroup in ('JUNIOR GIRLS', 'JR GIRLS', 'JUNION GIRLS');

update bnpid_old.CandidateInformationShared
set classgroup = 'Senior Men'
where classgroup in ('SENIOR MEN', 'MENS SPECIAL CLASS');

update bnpid_old.CandidateInformationShared
set classgroup = 'Senior Women'
where classgroup in ('SENIOR WOMEN', 'SENIOR WOMEN''S', 'SENI0R WOMEN', 'SENIOR WOMENS PANAWAGAN');

update bnpid_old.CandidateInformationShared
set classgroup = 'Seniors (Mixed)'
where classgroup in
      ('PANAWAGAN', 'SENIOR', 'APOSTOLIC', 'SENIOR CLASS', 'MIXED CLASS', 'APOSTOLIC MISSION', 'JOINT CLASS', 'COMBINED ADULT', 'SPECIAL CLASS', 'MIXED',
       'COMBINED CLASS', 'REGULAR', 'MIXED PANAWAGAN');